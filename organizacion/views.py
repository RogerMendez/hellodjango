...
@login_required(login_url='/user/login')
def unidad_plani(request, id_unidad):
    planificacion = Planificacion.objects.filter(unidad_id = id_unidad)
    return render_to_response('unidad/unid_plani.html',{'planificacion' :planificacion}, context_instance=RequestContext(request))
...
def option_plani(request):
    ...
    unidad = Unidades.objects.all()
    return render_to_response('unidad/option_plani.html',{'planificaciones' :planificacion, 'unidades' :unidad}, context_instance=RequestContext(request))
